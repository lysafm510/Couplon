...
def cytosolic_buffers_equation(c_ca_cyt, c_cam, c_trc, c_srm, c_slm):
    ...
    for i in range(0, NR):
        # CCACYT用的上一步的值
        c_cam[i] += (KPCAM * c_ca_cyt[i] * (BCAM - c_cam[i]) - KMCAM * c_cam[i]) * DT
        c_trc[i] += (KPTRC * c_ca_cyt[i] * (BTRC - c_trc[i]) - KMTRC * c_trc[i]) * DT
        c_srm[i] += (KPSRM * c_ca_cyt[i] * (BSRM - c_srm[i]) - KMSRM * c_srm[i]) * DT
        c_slm[i] += (KPSLM * c_ca_cyt[i] * (BSLM - c_slm[i]) - KMSLM * c_slm[i]) * DT
...
def cytosolic_ca_equation(c_ca_cyt, j_dye, j_buffers, c_ca_store, KRYR2, iteration):
    ...
    for k in range(0, iteration):
        ...
        #  全部点都结束后赋值
        new_c_ca_cyt = np.copy(med_c_ca_cyt)
    # 最后一次赋给CCACYT
    c_ca_cyt = np.copy(new_c_ca_cyt)
    return c_ca_cyt
...
def cytosolic_gn_equation(c_caf, j_dye, iteration):
    ...
    for k in range(0, iteration):
        ...
        new_c_caf = np.copy(med_c_caf)
    c_caf = np.copy(new_c_caf)
    return c_caf
